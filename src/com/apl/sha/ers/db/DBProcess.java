...
/*			" SELECT * FROM (\n" +
	...
	public boolean distributionDepot(int blnumber,User user)
	{
		...
		try {
			...
			prepareCall = conn.prepareCall(prodsql);
			prepareCall.setInt(1, blnumber);
			prepareCall.execute();
			result = true;
			setLog(user.getUserid(), "DistributionDepotAction", blnumber+";distribution result:"+result+";", new Date(), "info", "", "distribution depot");
		} catch (Exception e) {
			result = false;
			setLog(user.getUserid(), "DistributionDepotAction", blnumber+";distribution result:"+result+" errorMessage:"+e.getMessage()+";", new Date(), "info", "", "distribution depot");
		}finally{
			try {
				if (null != prepareCall) {
					prepareCall.close();
...
//				" from ERS.BOOKING b,ERS.BOOKINGSTATUS s where b.REFERENCENO=? and b.BLNUMBER = s.BLNUMBER order by s.RECEIVETIME asc";
		String sql = "select b.BLNUMBER as blnumber,b.BOOKINGPARTY as bookingparty,b.VESSELNAME as vesselName,b.VOYAGE as voyage,b.PACKAGES as packages," +
				" b.DESCRIPTIONOFPACKAGES as kindOfPackages,to_char(s.RECEIVETIME,'yyyy-MM-dd hh:mm:ss') as  receiveTime " +
				"from ERS.BOOKING b,ERS.BOOKINGSTATUS s where s.BLNUMBER = b.BLNUMBER and b.REFERENCENO = ? order by s.RECEIVETIME desc";
