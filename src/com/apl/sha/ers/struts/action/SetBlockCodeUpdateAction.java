 ...
 */
public class SetBlockCodeUpdateAction extends BaseAction {
	private static final String errorTag="error";
	private static final String cancelPath="/getBlockCodeUpdate.jsp";
	/*
	 * Generated Methods
	 */
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		BlockCodeUpdate bcu=this.getBlockCodeUpdate(request);		
		if(request.getParameter("delete_flag").toString().equalsIgnoreCase("Y")){
			//process.disableBlockCodeUpdate(bcu, user);
			process.setBlockCodeUpdate(bcu, 1, user);
		}
		else{
			process.setBlockCodeUpdate(bcu, 0, user);
		}
		request.getSession().setAttribute("BlockCodeUpdates",process.getBlockCodeUpdate());
		return mapping.findForward(success);
	}
	private BlockCodeUpdate getBlockCodeUpdate(HttpServletRequest request) {
		BlockCodeUpdate bcu=new BlockCodeUpdate();
		bcu.setId(Integer.parseInt(request.getParameter("id")));
		bcu.setSvcname(request.getParameter("svcname"));
		bcu.setPortofloadingcode(request.getParameter("portofloadingcode"));
		bcu.setBlock_code(request.getParameter("block_code"));
		return bcu;
	}
}